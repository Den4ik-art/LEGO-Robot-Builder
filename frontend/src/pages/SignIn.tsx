...
const loginImg = "/images/login_bg.png";
...
export default function SignIn() {
  ...
  return (
    <div className="min-h-screen w-full bg-[#facc15] flex items-center justify-center p-4 md:p-8 relative overflow-hidden font-sans">
      ...
      />
      <div className="w-full max-w-6xl h-auto min-h-[600px] md:min-h-[650px] bg-white rounded-3xl shadow-[16px_16px_0px_0px_#0f172a] relative overflow-hidden flex flex-col md:flex-row border-4 border-slate-900">

        <motion.div
          initial={{ x: "-100%" }} animate={{ x: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }}
          className="w-full md:w-1/2 h-full p-10 md:px-16 py-16 md:py-24 flex flex-col justify-center bg-white z-10 border-r-0 md:border-r-4 border-slate-900"
        >
          <div className="max-w-md w-full mx-auto">
            ...
            <div className="text-center mb-10 relative">
              <div className="inline-block bg-[#facc15] px-6 py-2 rounded-xl border-4 border-slate-900 shadow-[6px_6px_0px_0px_#0f172a] transform -skew-x-3 mb-4">
                <h2 className="text-3xl font-black italic uppercase text-slate-900 tracking-tighter">АВТОРИЗАЦІЯ <span className="text-red-600 drop-shadow-[2px_2px_0px_#0f172a]">TECHNIC</span></h2>
              </div>
              <p className="text-slate-900 font-black text-xs tracking-widest uppercase bg-white/80 inline-block px-4 py-1 rounded-lg border-2 border-slate-900 shadow-[2px_2px_0px_0px_#0f172a]">Повернення до лабораторії</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-6">
              <div>
                <label className="block text-slate-900 font-black uppercase tracking-widest text-xs mb-2">Ім'я користувача</label>
                <input type="text" name="username" value={form.username} onChange={handleChange} className="w-full px-4 py-4 rounded-xl border-4 border-slate-900 bg-white text-slate-900 shadow-[4px_4px_0px_0px_#0f172a] focus:shadow-[8px_8px_0px_0px_#facc15] focus:outline-none transition-all font-bold tracking-wider text-sm" required />
              </div>
              <div>
                <label className="block text-slate-900 font-black uppercase tracking-widest text-xs mb-2">Пароль</label>
                <input type="password" name="password" value={form.password} onChange={handleChange} className="w-full px-4 py-4 rounded-xl border-4 border-slate-900 bg-white text-slate-900 shadow-[4px_4px_0px_0px_#0f172a] focus:shadow-[8px_8px_0px_0px_#facc15] focus:outline-none transition-all font-bold tracking-wider text-sm" required />
              </div>
              <button type="submit" disabled={loading} className="w-full mt-8 bg-blue-600 text-white font-black uppercase tracking-widest text-lg py-4 rounded-xl border-4 border-slate-900 shadow-[6px_6px_0px_0px_#0f172a] hover:bg-blue-500 hover:translate-y-1 hover:translate-x-1 hover:shadow-[2px_2px_0px_0px_#0f172a] transition-all flex items-center justify-center gap-3">
                {loading ? <div className="w-6 h-6 border-4 border-white border-t-transparent rounded-full animate-spin" /> : <><FiLogIn size={24} />Ініціалізація</>}
              </button>
            </form>
            <div className="mt-8 text-center">
              <button onClick={() => navigate("/signup")} className="text-slate-900 bg-white border-4 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a] px-6 py-3 rounded-xl hover:bg-[#facc15] hover:translate-y-1 hover:translate-x-1 hover:shadow-[0px_0px_0px_0px_#0f172a] font-black text-xs tracking-widest uppercase transition-all flex items-center justify-center gap-2 w-full">
                Немає акаунту? Створити <FiArrowRight size={16} />
              </button>
            </div>
          </div>
        </motion.div>

        <motion.div initial={{ opacity: 0, x: "100%" }} animate={{ opacity: 1, x: 0 }} transition={{ type: "spring", stiffness: 300, damping: 30 }} className="hidden md:block absolute right-0 top-0 bottom-0 w-1/2 overflow-hidden bg-slate-100">
          <img src={loginImg} alt="Workshop" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-slate-900/60 to-transparent flex flex-col justify-end p-12">
            <div className="bg-slate-900/80 backdrop-blur-md p-8 rounded-2xl border-4 border-slate-900 shadow-[8px_8px_0px_0px_#facc15]">
              <h3 className="text-[#facc15] font-black italic uppercase text-3xl mb-4 tracking-tight drop-shadow-[2px_2px_0px_#0f172a]">З поверненням, Інженере</h3>
              <p className="text-white font-bold text-sm leading-relaxed uppercase tracking-widest">Майстерня готова. Отримайте доступ до своїх креслень та продовжуйте оптимізацію LEGO роботів.</p>
            </div>
          </div>
        </motion.div>
