...
class Component(Base):
    ...
    # --- Інженерні поля структурних компонентів ---
    engineering_role: Mapped[Optional[str]] = mapped_column(
        String(30), nullable=True, default="Structural"
    )  # Structural | Functional | Mechanical | Connection | Decorative
    volume_class: Mapped[Optional[str]] = mapped_column(
        String(20), nullable=True, default="Medium"
    )  # Small | Medium | Large
    structural_value: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True, default=1
    )  # Number of connection points the part provides
